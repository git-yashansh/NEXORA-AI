...
type ElectricBorderProps = PropsWithChildren<{
  color?: string;      // Main color of the electric border
  speed?: number;      // How fast the electric effect moves
  chaos?: number;      // Intensity/volume of electric sparks
...
const ElectricBorder: React.FC<ElectricBorderProps> = ({
  children,
  color = "#00F0FF",  // Electric cyan default
  speed = 1.5,          // Slightly faster
  chaos = 1.5,          // Higher intensity/volume
  thickness = 2,
  className,
  style,
}) => {
  ...
  // MANUAL radius change: adjust borderRadius here
  const inheritRadius: CSSProperties = {
    borderRadius: style?.borderRadius ?? "50%", // Circle
  };
  ...
  const bgGlowStyle: CSSProperties = {
    ...inheritRadius,
    transform: "scale(1.02)", // Slightly bigger than logo
    filter: "blur(20px)",
    opacity: 0.25,
    zIndex: -1,
  ...
  return (
    <div ref={rootRef} className={"relative isolate " + (className ?? "")} style={style}>
      {/* SVG filter for electric displacement */}
      <svg
        ref={svgRef}
